...
def test_cancel_during_final_mux_keeps_workspace_for_fragment_reuse(tmp_path) -> None:
    ...
    def concatenate(_fragments, *, destination, **_kwargs) -> None:
        destination.write_bytes(b"assembled fragment")
    ...
    with (
        ...
        patch("jasna.pipeline.concatenate_fragments", side_effect=concatenate),
        patch("jasna.pipeline.mux_final_output", side_effect=lambda *_a, **_k: first.cancel()),
    ):
    ...
    with (
        ...
        patch("jasna.pipeline.concatenate_fragments", side_effect=concatenate),
        patch("jasna.pipeline.mux_final_output", side_effect=lambda *_a, **_k: second.cancel()),
    ):
