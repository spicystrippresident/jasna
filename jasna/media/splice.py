...
def _source_video_contract(source: Path) -> tuple[str, float]:
    try:
        ...
        raise OutputValidationError(f"Could not inspect source video {source}: {exc}") from exc
    ...
    *,
    ...
    try:
        with av.open(str(output)) as container:
            if not container.streams.video:
                raise OutputValidationError(f"Completed output has no video stream: {output}")
            stream = container.streams.video[0]
            actual_codec = _canonical_codec(stream.codec_context.name)
            if expected_codec is not None and actual_codec != _canonical_codec(expected_codec):
            ...
            for packet in container.demux(stream):
                ...
                tail_seconds = max(tail_seconds or packet_seconds, packet_seconds + duration_seconds)
                inspected += 1
                if inspected >= 4096:
                    break
        ...
        raise OutputValidationError(f"Completed output is unreadable: {output}: {exc}") from exc
