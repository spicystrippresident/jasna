"""Minimal optional rocDecode bridge for the Linux AMD AV1 compatibility path.

The bridge is built lazily from the matching local ROCm SDK.  It deliberately
copies decoded NV12/P016 surfaces straight into Torch-owned GPU memory; there
is no host-memory transfer fallback in this module.
"""
...
class RocDecodeError(RuntimeError):
    """A rocDecode bridge error that should retain its native failure detail."""


def is_terminal_rocdecode_error(error: BaseException) -> bool:
    """Return whether continuing in the current ROCm context is unsafe."""
...
def _patch_helper_source(source: str) -> str:
    """Make rocDecode parser failures observable by the Python caller."""
...
def load_rocdecode_bridge() -> ctypes.CDLL:
    """Build and load the local bridge once per process."""
...
class RocDecoder:
    """One native decoder that copies each output surface D2D into Torch memory."""

    def __init__(self, device_id: int, codec_name: str):
        if not rocdecode_supported_codec(codec_name):
            raise RocDecodeError(
                f"rocDecode is available only on Linux for supported codecs, got {codec_name!r}"
            )
        self._handle = None
        codec = _CODECS[codec_name.lower()]
