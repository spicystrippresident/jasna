...
ZH = {
        ...
        "one_click_scan_threshold": "一键扫描阈值",
        "tip_one_click_scan_threshold": "仅用于预扫描和规划区间，不改变实际逐帧修复阈值。rfdetr-v6 默认 0.70，并结合连续命中过滤无码区间误报。",
        "one_click_confirmation": "连续命中确认",
        "tip_one_click_confirmation": "要求相邻采样点连续达到扫描阈值，用于过滤孤立误报。短暂马赛克素材可改为 1 次。",
        "one_click_confirmation_one": "1 次（灵敏）",
        "one_click_confirmation_two": "2 次（推荐）",
        "one_click_confirmation_three": "3 次（严格）",
        "section_basic": "基本处理",
        ...
        "denoise_step": "降噪应用时机",
        "save_run_log": "保存诊断运行日志",
        "run_log_path": "诊断运行日志：{path}",
        "run_log_unavailable": "无法保存诊断运行日志。处理将继续。",
        "denoise_none": "无",
        "denoise_low": "低",
        "denoise_medium": "中",
        "denoise_high": "高",
        ...
        "tip_save_run_log": "为本批任务写入尽力而为的诊断日志。选择输出文件夹时，日志保存在 .jasna-logs；使用“与输入相同”时，日志保存在 Jasna 配置目录的 run-logs 中。日志包含队列输入、处理消息以及定期的 Linux 主机/GPU 遥测。记录日志不会改变处理行为。\n\n默认值：关闭",
