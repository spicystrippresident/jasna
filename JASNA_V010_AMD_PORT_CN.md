...
6. 队列中的处理项目会显示当前阶段：准备、粗扫、精扫、源视频复制、修复或成片整理。阶段旁的倒计时明确标为“阶段 ETA”，只表示当前阶段预计剩余时间，不表示整个视频距离完成的时间。
...
- 任务前精扫直接按可设置的采样网格生成最终候选区间，不再执行第二轮逐帧边界补扫；自动扩边跟随该网格并限制在 `0.5～1.0s`，额外完整解码的成本高于它能增加的边界精度。
...
- 大于 `10s` 的精扫候选继续使用用户设置的检测阈值，避免丢失真实长段的弱边界；不超过 `1s` 的候选忽略；其余短候选只有包含连续 `2s`、分数不低于 `0.90` 的高置信度核心才保留。该过滤发生在扩边之前。
- 精扫扩边由 `pre_scan_pad_seconds` 设置。默认 `auto` 跟随精扫取样间隔，并限制在 `0.5～1.0s`；也可明确选择 `0`、`0.5`、`1`、`2` 或 `5s`。
- 取消旧的最短 `30s` 区间和 `30s` 间隔合并。扩边后只合并重叠或相接区间；落到同一关键帧范围的区间仍由 Smart Render 自动合并。
...
- 短段规则与间隔策略使用一段真实的 48 分钟、8192x4096、59.94 fps 视频的持久化精扫断点离线复算：原检测阈值得到 167 段、共 1117.617s；置信度过滤后保留用户确认的 7 段、共 981.981s；自动前后 0.5s 扩边后为 6 个不重叠效果区间、共 988.482s。结合源视频真实关键帧，不做 30 秒间隔合并时为 5 个渲染片段、共 1003.135s；保留合并时为 4 个片段、共 1023.155s。少启动一个片段却多修复 20.020s，因此采用不做额外间隔合并。该验证复用了已有分数与关键帧，没有重新解码或占用 GPU。
...
- 本轮短段置信度过滤与可设置扩边增加了“视频末尾候选必须按实际截断时长计算”的回归。精扫、协调器、停止、隔离任务、扫描器和设置聚焦测试为 `76 passed, 1 deselected`；排除项是已记录、与本改动无关的平台默认 CQ 基线。完整本地化契约 `158/158` 通过，`compileall` 与 `git diff --check` 通过。
- 队列阶段信息已贯通扫描回调和隔离视频任务协议；协调器、处理器、隔离任务与队列行聚焦测试 `42/42` 通过，完整本地化契约仍为 `158/158`。存在明确阶段时，队列倒计时显示为“阶段 ETA”。
- 在后续短段归一化调整之前，已用同一份 8192×4096、59.94 fps、10-bit HEVC SBS 素材完成三路真实验收：无码短片粗扫覆盖率 `0%` 并直接复制；强有码短片覆盖率 `100%` 并全片修复；51.515 秒过渡片粗扫覆盖率 `37.8%` 并进入精扫。当时将其 103 个真实精扫采样点按直接网格策略重放后，得到 `21.515–51.515s`、`58.24%` 覆盖率的 Smart Render 区间。
...
- 普通 2D H.264 已用一段真实 1920×1080、29.97 fps、`yuv420p` 8-bit 短片验收（`31.7407s`、951 个视频帧、1,366 个 AAC 包）。4 秒自动粗扫通过 4 次 GOP seek 取得 11 个采样，覆盖率 `52.6%`，随后进入 0.5 秒精扫；置信度归一化最终保留 `12.513–26.526s`（`44.2%`）并路由 Smart Render。扫描耗时 `12.86s`，隔离 Smart Render 耗时 `56.65s`，关键帧对齐后实际处理 701 帧；另一次“不单独扫描”全片路线耗时 `76.06s`。两份输出都保持 951 个视频帧、1,366 个 AAC 包和 `31.7317s` 视频时间线，严格完整解码无错误。全片路线峰值显存约 2.8 GiB，无 offload。Smart Render seek 时打印过一次 AMF H.264 parser assertion，但之后没有帧数、时长或解码不一致；保留为后续多素材观察项。
- 普通 2D AV1 已用一段真实 1920×1080、23.976 fps、AV1 Main `yuv420p10le` 10-bit 短片验收（`38.829s`、931 个视频帧、1,819 个 AAC 包）。4 秒自动粗扫通过 4 次 GOP seek 取得 12 个采样，耗时 `11.55s`，覆盖率 `0%`，正确选择源文件直拷；隔离直拷耗时 `3.52s`。另一次“不单独扫描”AV1 AMF 全片路线耗时 `30.56s`；手动选择 `12–16s` 的 Smart Render 扩展为一个关键帧 GOP，处理 240 帧，耗时 `14.36s`。直拷、全片和 Smart Render 输出都保持 931 个视频帧和 1,819 个 AAC 包；全片与 Smart Render 仍为 AV1 Main 10-bit，BT.709 标记不变，时长与源相差不超过 1ms，并通过严格完整解码。全片处理峰值显存约 2.3 GiB，无 offload。
- AV1 成功路线前重复出现的 `hardware accelerated decoding` / `Missing Sequence Header` 不是正式生产解码器输出。该源文件不含 `nb_frames`，元数据收集因而回退到 OpenCV `CAP_PROP_FRAME_COUNT`；单独运行该 OpenCV 探测即可复现全部提示，但仍正确返回 931 帧。正式读取器随后在 Linux AMD 上明确选择 FFmpeg 软件 AV1 解码并上传 ROCm。它属于需要后续修复的元数据探测日志噪声，不是丢帧证据。全部测试串行结束后，系统显存回落到约 1.0 GiB，可用内存约 20 GiB。
- 过渡片此前带补扫的自动区间为 `21.468–51.468s`，隔离 Smart Render 实际处理 1888 帧；最终输出与源均为 3085 帧、8192×4096、HEVC Main 10、`yuv420p10le`，输出/源时长分别为 `51.518s` 与 `51.515s`。当时的直接网格重放仅将补扫边界改变 `0.047s`，小于一个精扫采样间隔；该旧输出继续作为时间线和拼接证据，当前区间归一化以上文第 9 节为准。
- 初版逐帧边界请求在真实 8K AMD 测试中复现系统桌面卡死；上一启动内核日志明确为 `amdgpu_cs_ioctl: Not enough memory for command submission`。连续窗口解码和复用 rocDecode 虽修复显存累积，但随后一部完整视频显示该阶段仍不值得保留：`0.5s` 精扫检测 5,755 个采样点耗时 `16m54s`，其后的 334 个边界窗口又检测 9,710 帧并耗时 `22m47s`。按当时规则用同一精扫检查点直接生成区间仍得到相同 7 段，各起止点仅相差 `0.18～0.43s`，覆盖率只相差约 `0.0035` 个百分点；取消补扫可将该次扫描路线从 `42m29s` 降至约 `19m42s`，约快 `2.16` 倍。因此自动预扫描已取消边界窗口解码并保留可续扫的 `0.5s` 断点；后续启用的短段置信度过滤、可设置扩边和不做额外间隔合并以上文第 9 节为准。
- 自动三路真实验收仍由 10-bit HEVC SBS 覆盖；普通 2D 现已额外覆盖自动 H.264 Smart Render、强制 H.264 全片、自动 AV1 直拷，以及强制 AV1 全片/Smart Render。强制路线证明处理和拼接行为，但不等同于自动路由判断覆盖。剩余缺口包括普通 2D HEVC、AV1 8-bit、4K/8K AV1、自动 H.264 直拷/全片判断，以及自动 AV1 Smart Render/全片判断；本次获准素材中没有可用的 2D HEVC。生成视频、扫描断点和本机媒体路径均未加入仓库。
...
1. 8-bit NV12/yuv420p：普通 2D H.264 的自动 Smart Render 与强制全片已完成；仍需补自动 `copy/full` 判断。
2. 10-bit P010/yuv420p10：自动三路工程验收已完成；仍需用户做最终画质人眼确认和长片批处理。
3. SBS fisheye VR：有码区间的连续运动、贴回稳定性与无码区间复制。
4. 普通 2D：H.264 与 AV1 的无投影处理和智能分段拼接已完成；仍缺 HEVC。
5. H.264、AV1 已有短片工程验收；仍需 HEVC 2D、AV1 8-bit 和高分辨率 AV1。
