...
- 已完成当前代码的真实 8-bit 智能分段验收：同一 20 秒 8K 素材只恢复 `5–6s`，SBS fisheye、batch 8、最大片段 180。进程返回 0，输出与源文件同为 1201 个 HEVC 视频帧/包和 940 个 AAC 包，时长分别为 `20.036678s` 与 `20.036683s`，完整软件解码无错误，结束后显存回落到约 `1.76–1.80 GiB`。
