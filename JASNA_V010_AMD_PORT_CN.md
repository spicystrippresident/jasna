...
- 边界补扫按窗口连续逐帧解码和批量检测，不再为每一帧重建 8K reader；随机 mask、投影比较和边界窗口共用 worker 级 `ReusableRocDecoder`，避免 AMD native surface pool 累积。
...
- 自动模式默认每 `2.0s` 粗扫一帧。无命中直接用 FFmpeg 原子 remux 源视频；采样覆盖率达到默认 `85%` 时直接全片修复；其余情况进入精扫。覆盖率阈值和粗扫间隔均可设置。
- 扫描模式不运行粗扫，默认每 `0.5s` 精扫一帧；自动模式进入精扫后使用同一间隔。精扫间隔可设置。
- 精扫命中组的左右边界按连续源帧补扫，并要求连续两帧确认；随后复用 v0.9.1 已验证的前后 `5s` 扩边、`30s` 最短区间和 `30s` 间隔合并规则。
- 最终区间为空时直接复制；覆盖全片时走全片修复；其他情况复用现有 `segments + Smart Render`。自动区间遇到 Smart Render 不兼容时回退全片，手动区间仍保持严格失败语义。
- 扫描断点默认落盘，不增加“是否落盘”选项。签名绑定源文件、输出路径、检测模型与权重、阈值、FP16、VR 模式和扫描参数；按 detector batch 保存精确 PTS，可在停止、异常或重启后继续。
...
- 因此目前没有证据支持“v0.10 双 rocDecode 销毁路径必然触发 panic”。此前历史 panic 在 8 次受控尝试中未复现；它与下述已定位的边界补扫显存耗尽不是同一个结论。
...
- 自动预扫描专项回归为 `214 passed`，并通过 `py_compile` 与 `git diff --check`；覆盖三路路由、设置、断点精确 PTS、停止/关闭、隔离协议、Smart Render 回退和 locale 契约。
- 已用同一份 8192×4096、59.94 fps、10-bit HEVC SBS 素材完成三路真实验收：无码短片粗扫覆盖率 `0%` 并直接复制；强有码短片覆盖率 `100%` 并全片修复；51.515 秒过渡片粗扫覆盖率 `37.8%`，精扫和边界补扫后以 `58.3%` 覆盖率进入 Smart Render。
- 过渡片自动区间为 `21.468–51.468s`，隔离 Smart Render 实际处理 1888 帧；最终输出与源均为 3085 帧、8192×4096、HEVC Main 10、`yuv420p10le`，输出/源时长分别为 `51.518s` 与 `51.515s`。
- 初版逐帧边界请求在真实 8K AMD 测试中复现系统桌面卡死；上一启动内核日志明确为 `amdgpu_cs_ioctl: Not enough memory for command submission`。改为连续窗口解码和复用 rocDecode 后，边界阶段数秒完成，端到端运行无新增 AMDGPU 错误；系统侧观察显存峰值约 `17.5 GB / 25.8 GB`，进程退出后回落约 `1.64 GB`。
