...
- 完整视频实测表明旧粗扫即使采用稀疏步长仍会顺序解码整片，因此默认目标间隔由 `2.0s` 调整为 `4.0s`，并改用自适应关键帧/GOP 路线跳过无目标 GOP。该变化只影响自动模式的初始覆盖率估算；进入精扫后仍使用 `0.5s` 取样并连续补扫边界。
- 扫描断点默认落盘，不增加“是否落盘”选项。签名绑定源文件、输出路径、检测模型与权重、阈值、FP16、VR 模式、扫描参数、自适应 GOP 策略、容差比例和覆盖率算法；按 detector batch 保存精确 PTS，可在停止、异常或重启后继续，旧粗扫断点会安全失效。
