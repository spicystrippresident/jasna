...
| キー | 説明 |
| --- | ------------ |
| `cq` | 数値を変更せず AMF の `qvbr_quality_level` として渡す品質目標。低いほど高品質。範囲は 0–51、デフォルトは 24（H.264）、25（HEVC）、32（AV1）。Linux と Windows の AMD 10 ビット AV1 は上記のソースレート連動ピーク VBR の例外を使います。 |
...
| `rc` | レート制御モード。デフォルト `qvbr`。Linux と Windows の AMD 10 ビット AV1 は `vbr_peak` に固定されます。 |
| `preset` | AMF プリセット。 |
| `g` | キーフレーム間隔（フレーム数）。デフォルト 250。 |
| `bf` | 連続 B フレームの最大数。 |
| `preanalysis` | 事前分析パス。デフォルトで有効ですが、Linux と Windows の AMD 10 ビット AV1 では無効になります。 |
