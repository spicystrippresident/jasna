...
Linux AMD の 10 ビット AV1 は例外です。AMF は P010 と PreAnalysis を同時に使用できず、
PreAnalysis を無効にした QVBR はビットレート上限を守りません。そのため Jasna はこの組み合わせで
ソースレート連動のピーク VBR を使用します。`--cq` はインターフェース互換性のため受け付けますが、
QVBR の品質目標としては適用されません。
