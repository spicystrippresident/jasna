...
| キー | 説明 |
| --- | ------------ |
| `cq` | 汎用の品質目標。低いほど高品質です。H.264/AV1 は値を変更せず `qvbr_quality_level` に渡します。HEVC の全体処理は値をそのまま CQP の I/P QP に使い、Linux AMD の HEVC Smart Render 断片は不安定な断片 QVBR を避けるため、0–51 に制限した `CQ + 2` を使います。範囲は 0–51、デフォルトは 24（H.264）、25（HEVC）、32（AV1）。 |
...
| `bf` | 連続 B フレームの最大数。 |
| `preanalysis` | 事前分析パス。H.264/AV1 はデフォルトで有効、HEVC のデフォルト CQP と Linux AMD HEVC Smart Render 断片では無効です。 |
